...
export const reminderDispatcher: HttpFunction = async (req, res) => {
  ...
  try {
    ...
    console.error('[reminder-dispatcher] Unhandled error:', err)
...
async function dispatchFollowups(now: Date): Promise<number> {
  // Window: bookings that ended between 23h and 25h ago
  ...
  for (const booking of bookings) {
    try {
      const reviewToken = generateReviewToken(booking.id)
      ...
      console.error('[followup] Failed for booking:', booking.id, err)
    }
  }
  return sent
}
...
async function sendReminderEmail(booking: any, type: '24h' | '1h') {
  ...
  const subject =
    type === '24h'
      ? `Reminder: ${service.name} tomorrow at ${formattedTime}`
      : `Starting in 1 hour: ${service.name} at ${formattedTime}`
...
async function sendFollowupEmail(booking: any, reviewToken: string) {
  ...
  // Use Google review URL if set, otherwise internal review page
  const reviewUrl = business.google_review_url
    ? business.google_review_url
    : `${process.env.APP_URL}/${business.slug}/review?token=${reviewToken}`
...
<body style="margin:0;padding:0;background:#FAFAF8;font-family:Arial,sans-serif;">
  <div style="max-width:520px;margin:40px auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #EBEBEB;">
    ...
    <div style="padding:28px 32px;">
      ...
      <p style="color:#CCC;font-size:11px;text-align:center;margin:0;">Powered by Your Business in a Link</p>
    </div>
  </div>
</body>
</html>`.trim()
  ...
  <div style="max-width:520px;margin:40px auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #EBEBEB;">
    ...
    <div style="padding:28px 32px;text-align:center;">
      <p style="color:#555;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Hi ${data.customerName}, we'd love to hear how your session went. It only takes 30 seconds.
      </p>
...
function generateReviewToken(bookingId: string): string {
  const payload = Buffer.from(
    JSON.stringify({ bookingId, iat: Date.now() })
  ).toString('base64url')

  const sig = crypto
    .createHmac('sha256', process.env.REVIEW_TOKEN_SECRET!)
    .update(payload)
    .digest('base64url')
