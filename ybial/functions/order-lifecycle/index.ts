...
export const orderLifecycle: HttpFunction = async (req, res) => {
  ...
  try {
    // Idempotency — payment_intent_id is UNIQUE on orders table
    ...
    console.error('[order-lifecycle]', err)
  ...
  const lineItemsHtml = data.items
    .map(
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid #EAE5DC;">
        <span style="color:#111;font-size:13px;">${item.emoji} ${item.name} × ${item.quantity}</span>
  ...
  <div style="max-width:520px;margin:40px auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #EBEBEB;">
    ...
    <div style="padding:28px 32px;">
      ...
      <p style="color:#888;font-size:13px;text-align:center;margin:0 0 24px;">
        The business will be in touch about your order.
      </p>
      ...
      <p style="color:#CCC;font-size:11px;text-align:center;margin:16px 0 0;">Powered by Your Business in a Link</p>
    </div>
  </div>
</body>
</html>`.trim()
