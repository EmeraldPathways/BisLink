...
async function handleBookingPayment(
  ...
  const { data: existingBooking, error: existingBookingError } = await supabase
    .from('bookings')
    .select(
      'id, business_id, start_time, amount_paid, customer_name, customer_email, status, payment_status, confirmation_sent, google_event_id, payment_intent_id',
    )
  ...
  if (isAlreadyPaid) {
    ...
    if (
      !existingBooking.confirmation_sent ||
  ...
  const { data: booking, error } = await supabase
    ...
    .eq('payment_intent_id', intent.id)
    .select(
      'id, business_id, start_time, amount_paid, customer_name, customer_email',
    )
    .single();
  ...
  if (error || !booking) {
    logWebhook(
      ...
      {
        payment_intent_id: intent.id,
        error: error?.message ?? 'Unknown booking update error',
      },
      'error',
    );
    return;
  ...
  const { error: customerStatsError } = await supabase.rpc(
...
async function handlePaymentFailed(
  ...
  if (type === 'product_order' || type === 'order') {
    ...
    if (error) {
      logWebhook(
        'order_payment_failed_update_failed',
        { payment_intent_id: intent.id, error: error.message },
        'error',
      );
    }
    return;
...
async function handleAccountUpdated(
  ...
  if (error) {
    logWebhook(
      'stripe_account_update_failed',
      { stripe_account_id: account.id, error: error.message },
      'error',
    );
  }
}

async function triggerBookingLifecycle(
  bookingId: string,
  ...
  if (!url) {
    logWebhook(
      'booking_lifecycle_missing_config',
      { payment_intent_id: paymentIntentId ?? null, booking_id: bookingId },
      'warn',
    );
    return;
  }
...
async function triggerOrderLifecycle(payload: {
  ...
  if (!url) {
    logWebhook(
      'order_lifecycle_missing_config',
      {
