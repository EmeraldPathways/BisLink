    ...
    <div className="min-h-screen bg-[var(--color-bg)] md:flex">
      <aside className="hidden w-60 flex-col justify-between border-r border-[var(--color-border)] bg-white p-5 md:flex">
        <div>
          ...
          <nav className="mt-8 space-y-1">
            {navItems.map(({ label, href, icon: Icon }) => (
              <Link
                key={label}
                className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium hover:bg-[var(--color-surface-2)]"
                href={href}
              >
        ...
        <div className="space-y-3">
          <Link
            href="/availability"
            className="flex items-center gap-3 rounded-xl border border-[var(--color-border)] px-3 py-3 text-sm font-medium"
          >
          ...
          <Link
            href={`/${business.slug}`}
            className="block rounded-2xl bg-[var(--color-void)] px-4 py-3 text-center text-sm font-semibold text-white"
          >
            View My Link -&gt;
          </Link>
          <div className="rounded-2xl bg-[var(--color-surface-2)] p-4">
            ...
            <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
              {user.email ?? business.email ?? 'Owner account'}
            </p>
          </div>
        </div>
      </aside>
      <main className="flex-1 overflow-x-hidden px-5 pb-[calc(env(safe-area-inset-bottom)+6.5rem)] pt-6 md:px-8 md:pb-8">
        {children}
      </main>
      <MobileNav
        business={business}
