...
export default async function AdminFinancePage() {
  ...
  const { totals, businessesNeedingAttention, topBusinesses } =
  ...
  return (
    <div className="space-y-6">
      <AdminTopbar
        title="Finance"
        description="Track platform revenue, Stripe account readiness, payout flow, and the businesses that need financial intervention."
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          label="Month revenue"
          value={formatPrice(totals.monthRevenue)}
        />
        <Metric
        ...
        <Metric
          ...
          value={String(totals.inTransitPayouts)}
        />
      </div>
          ...
          <div className="mt-5 space-y-3">
            {topBusinesses.map((entry) => (
              <Link
                ...
                href={`/admin/businesses/${entry.business.id}`}
                className="flex items-center justify-between rounded-[20px] border border-[var(--color-border)] p-4"
              >
                <div>
          ...
          <div className="mt-5 space-y-3">
            {businessesNeedingAttention.map((business) => (
              <Link
                ...
                className="block rounded-[20px] border border-[var(--color-border)] p-4"
              >
                <p className="text-sm font-semibold">{business.name}</p>
                <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
                  {business.stripe_onboarded
                    ? 'Missing Stripe account data'
