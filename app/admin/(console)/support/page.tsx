    ...
    <div className="space-y-6">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
                      {review.rating}/5 • {review.text || 'No written feedback'}
                    </p>
                  </div>
                  <AdminActionButton
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <ListSection
          ...
          items={refundedOrders.map(
            (order) =>
              `${order.customer_name} • ${order.status} • ${new Date(order.created_at).toLocaleString()}`,
          )}
          empty="No refunded orders."
        />
        <ListSection
          title="Booking issues"
          items={bookingIssues.map(
            (booking) =>
              `${booking.customer_name} • ${booking.status}/${booking.payment_status} • ${new Date(booking.start_time).toLocaleString()}`,
          )}
      ...
      <div className="mt-5 space-y-3">
        {items.length ? (
          items.map((item) => (
            <div
              key={item}
              className="rounded-[20px] bg-[var(--color-surface-2)] p-4 text-sm"
            >
              {item}
            </div>
          ))
