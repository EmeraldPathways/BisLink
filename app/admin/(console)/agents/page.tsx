...
export default async function AdminAgentsPage() {
  ...
  return (
    <div className="space-y-6">
      <AdminTopbar
        title="Agent area"
        description="Track the internal AI workers that support booking, support, business advice, and operational monitoring."
      />

      <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--color-gold-dark)]">
              Overall status
            </p>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <SummaryCard
              label="Warn"
              value={String(diagnostics.summary.warn)}
            />
            <SummaryCard
              label="Fail"
              value={String(diagnostics.summary.fail)}
            />
          </div>
        </div>
      </div>
            ...
            {agent.details.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {agent.details.map((detail) => (
                  <span
                    key={detail}
                    className="rounded-full bg-[var(--color-surface-2)] px-3 py-1 text-[11px] font-semibold text-[var(--color-text-secondary)]"
                  >
                    {detail}
                  </span>
                ))}
