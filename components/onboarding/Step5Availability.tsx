...
export function Step5Availability() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-gold-dark)]">Step 5 of 7</p>
        <h2 className="mt-2 font-display text-5xl">Your availability</h2>
      </div>
      {days.map((label, index) => {
        const record = demoAvailability.find((item) => item.day_of_week === index);
        return (
          <div key={label} className="grid grid-cols-[70px_80px_1fr_1fr] items-center gap-3 rounded-[22px] border border-[var(--color-border)] bg-white px-4 py-4">
