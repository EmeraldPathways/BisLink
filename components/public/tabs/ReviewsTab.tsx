  ...
  return (
    <section id={id} className="scroll-mt-20 space-y-4 px-2 pb-10 pt-6">
      ...
      <div className="rounded-[var(--card-radius)] bg-[image:var(--hero-gradient)] px-5 py-6 text-center text-[var(--hero-text)]">
        <p className="font-display text-[64px] leading-none tracking-[-2px]">{reviewSummary.average.toFixed(1)}</p>
        <div className="mt-2">
          <StarRating rating={reviewSummary.average} />
        </div>
        <p className="mt-3 text-[13px] text-[var(--hero-text-muted)]">Based on {reviewSummary.publishedCount} reviews</p>
            ...
            <div key={item.rating} className="flex items-center gap-3 text-sm">
              ...
              <div className="h-1.5 flex-1 rounded-full bg-white/10">
                <div
                  ...
                  style={{ width: item.count === 0 ? '0%' : `max(${Math.min(item.percent * 100, 100)}%, 4px)` }}
                />
              </div>
              <span className="w-10 text-right text-[12px] text-[var(--hero-text-muted)]">{Math.round(item.percent * 100)}%</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {published.map((review) => (
        <div key={review.id} className="rounded-[var(--card-radius)] border-[1.5px] border-[var(--border)] bg-[var(--page-card-bg)] p-4 shadow-[var(--card-shadow)]">
          ...
          <p className="mt-3 text-[13px] leading-[1.65] text-[var(--text-2)]">&ldquo;{review.text}&rdquo;</p>
        </div>
      ))}
      ...
      <div className="rounded-[var(--card-radius)] bg-[var(--page-surface-muted)] px-5 py-5 text-center">
        ...
        <p className="mt-2 text-sm text-[var(--text-3)]">Join {reviewSummary.publishedCount}+ people who&apos;ve worked with {business.name}.</p>
        <button type="button" onClick={onBook} className="mt-4 rounded-[var(--button-radius)] bg-[var(--cta-bg)] px-5 py-3 text-sm font-semibold text-[var(--cta-text)]">
          Book a session
        </button>
        {business.google_review_url ? (
          <a href={business.google_review_url} target="_blank" rel="noreferrer" className="mt-3 block text-sm font-semibold text-[var(--accent-strong)]">
            Leave a review
          </a>
        ) : null}
      </div>
