  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          {labels.map((label, index) => {
            ...
            return (
              <div key={label} className="grid grid-cols-[80px_80px_1fr_1fr] items-center gap-3 rounded-[18px] bg-[var(--color-surface-2)] px-4 py-4">
                ...
                <button className={`rounded-full px-3 py-1 text-xs font-semibold ${record ? 'bg-[var(--color-void)] text-white' : 'bg-white text-[var(--color-text-secondary)]'}`}>
                  {record ? 'On' : 'Off'}
                </button>
      ...
      <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-5">
        ...
        <div className="mt-5 space-y-3">
          <input className="w-full rounded-xl border border-[var(--color-border)] px-4 py-3" type="date" />
          <div className="grid grid-cols-2 gap-3">
            <input className="w-full rounded-xl border border-[var(--color-border)] px-4 py-3" type="time" />
            <input className="w-full rounded-xl border border-[var(--color-border)] px-4 py-3" type="time" />
          </div>
          <input className="w-full rounded-xl border border-[var(--color-border)] px-4 py-3" placeholder="Reason" />
          <button className="w-full rounded-2xl bg-[var(--color-void)] px-4 py-3 text-sm font-semibold text-white">Add blocked time</button>
        </div>
          ...
          {blockedTimes.map((blocked) => (
            <div key={blocked.id} className="rounded-[18px] bg-[var(--color-surface-2)] px-4 py-4">
              ...
              <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
                {new Date(blocked.start_time).toLocaleString()} to {new Date(blocked.end_time).toLocaleString()}
              </p>
            </div>
          ))}
        </div>
